# -*- coding: utf-8 -*-
"""final dashboard.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/10fIAYaZSVCar8fTKx7fTNG_qlvmhBJtg
"""
...
# from sklearn.cluster import KMeans
# 
# # 1. PAGE SETUP & ELITE THEME
# st.set_page_config(page_title="Milma Strategic Intelligence", layout="wide")
# 
# st.markdown("""
#     <style>
#     .stApp { background: linear-gradient(180deg, #011425 0%, #242424 100%); }
#     div[data-testid="stMetric"], .stDataFrame, .stPlotlyChart {
#         background: rgba(31, 73, 89, 0.15) !important;
#         border: 1px solid #5C7C89 !important;
...
#         box-shadow: 0 4px 30px rgba(0, 0, 0, 0.5) !important;
#     }
#     [data-testid="stSidebar"] { background-color: #011425 !important; border-right: 1px solid #1F4959; }
#     h1, h2, h3, p, span { font-family: 'Segoe UI', sans-serif !important; color: #FFFFFF !important; }
#     </style>
# """, unsafe_allow_html=True)
# 
# # 2. DATA ENGINE
...
#         df.columns = [str(c).strip().lower() for c in df.columns]
#         p_col = next((c for c in df.columns if any(x in c for x in ['product', 'item', 'particulars'])), df.columns[0])
#         q_col = next((c for c in df.columns if any(x in c for x in ['qty', 'quantity'])), None)
#         s_col = next((c for c in df.columns if any(x in c for x in ['sales', 'amt', 'value'])), None)
#         r_col = next((c for c in df.columns if any(x in c for x in ['rate', 'price'])), None)
#         date_col = next((c for c in df.columns if 'date' in c), None)
# 
#         if not q_col or not s_col: continue
# 
...
#         else:
#             df['date_dt'] = pd.to_datetime("2025-01-01")
# 
#         df = df.dropna(subset=['date_dt']).copy()
#         clean = pd.DataFrame({
#             'product': df[p_col].astype(str).str.strip(),
#             'qty': pd.to_numeric(df[q_col], errors='coerce').fillna(0),
#             'sales': pd.to_numeric(df[s_col], errors='coerce').fillna(0),
#             'price': pd.to_numeric(df[r_col], errors='coerce').fillna(1) if r_col else (df[s_col]/df[q_col].replace(0,1)),
#             'date': df['date_dt'],
#             'year': df['date_dt'].dt.year
#         })
#         all_data.append(clean)
...
#     if not df.empty:
#         menu = st.sidebar.radio("Navigation", ["Executive Summary", "Strategic Recommendations", "Risk & Forecast"])
#         v = df.copy()
# 
#         if menu == "Executive Summary":
#             st.title("Performance Overview")
#             c1, c2, c3 = st.columns(3)
#             c1.metric("Active Products", v['product'].nunique())
#             c2.metric("Total Revenue", f"₹{v['sales'].sum():,.0f}")
#             c3.metric("Total Qty", f"{int(v['qty'].sum()):,}")
#             st.plotly_chart(px.line(v.groupby('date')['sales'].sum().reset_index(), x='date', y='sales', title="Revenue Trend"), use_container_width=True)
# 
#         elif menu == "Strategic Recommendations":
#             st.title(" Strategic Product Audit")
# 
...
# 
#         elif menu == "Risk & Forecast":
#             st.title("2026 Projections")
#             st.info("Forecasting Module Active")
# else:
#     st.info("Upload CSV files to begin.")
